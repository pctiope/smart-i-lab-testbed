...
from air1_all_zones.feature_contract import ALL_ZONE_IDS  # noqa: E402
...
def _empty_ground_truth_by_zone() -> dict[str, dict[str, Any]]:
    return {
        str(zone_id): {
            "count": None,
            "occupied": None,
...
class TickEvent:
    ...
    reference_time: str
    source_label: str
    zone_probabilities: dict[int, float] = field(default_factory=dict)
    aggregate_probability: float | None = None
    occupied_zones: list[int] = field(default_factory=list)
    zone_count: int = 0
    ...
    ground_truth_by_zone: dict[str | int, dict[str, Any]] = field(default_factory=dict)
    error: str | None = None

    def to_dict(self) -> dict[str, Any]:
        valid_zone_probabilities = {
            int(zone_id): probability
            for zone_id, probability in self.zone_probabilities.items()
            if int(zone_id) in ALL_ZONE_IDS
        }
        valid_occupied_zones = [
            int(zone_id)
            for zone_id in self.occupied_zones
            if int(zone_id) in ALL_ZONE_IDS
        ]
        ground_truth_by_zone = _empty_ground_truth_by_zone()
        for zone_id, label in self.ground_truth_by_zone.items():
            key = str(zone_id)
            if key not in ground_truth_by_zone or not isinstance(label, dict):
                continue
            ground_truth_by_zone[key] = {
                "count": _json_safe_float(label.get("count")),
                "occupied": label.get("occupied"),
                "timestamp": label.get("timestamp"),
                "age_minutes": _json_safe_float(label.get("age_minutes")),
            }
        return {
            ...
            "zone_probabilities": {
                str(zone_id): _json_safe_float(probability)
                for zone_id, probability in sorted(valid_zone_probabilities.items())
            },
            "aggregate_probability": _json_safe_float(self.aggregate_probability),
            "occupied_zones": valid_occupied_zones,
            "zone_count": len(valid_zone_probabilities) if valid_zone_probabilities else int(self.zone_count),
            ...
            "ground_truth_by_zone": ground_truth_by_zone,
            "error": self.error,
        }
...
class InferenceLoop:
    ...
    def _predict_blocking(self) -> tuple[pd.DataFrame, pd.Timestamp, dict[int, float]]:
        ...
        return window, latest_ts, {int(zone_id): float(prob) for zone_id, prob in probabilities.items()}

    async def _tick_once(self) -> None:
        if self.state.run_dir is None:
            return
        try:
            _window, latest_ts, probabilities = await asyncio.to_thread(self._predict_blocking)
        ...
        finite_probabilities = {
            int(zone_id): float(probability)
            for zone_id, probability in probabilities.items()
            if int(zone_id) in ALL_ZONE_IDS and math.isfinite(float(probability))
        }
        if not finite_probabilities:
            return
        probability = max(finite_probabilities.values())
        aggregate_probability = sum(finite_probabilities.values()) / len(finite_probabilities)
        threshold = self.config.threshold
        occupied_zones = [
            int(zone_id)
            for zone_id, zone_probability in finite_probabilities.items()
            if zone_probability >= (threshold if threshold is not None else 0.5)
        ]
        occupied = bool(occupied_zones) if threshold is not None else None
        ...
        event = TickEvent(
            ...
            source_label=self.data_source.label,
            zone_probabilities=finite_probabilities,
            aggregate_probability=aggregate_probability,
            occupied_zones=occupied_zones,
            zone_count=len(finite_probabilities),
            **ground_truth_fields,
            error=None,
        )
        self.history.append(event)
