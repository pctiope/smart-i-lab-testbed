...
def build_all_zones_base_feature_frame(
            ...
            rows.append(
                {
                    ...
                    "temp": air_row.get("temps", {}).get(zone_id),
                    "rh": air_row.get("rhs", {}).get(zone_id),
                    "co2": air_row.get("co2s", {}).get(zone_id),
                    "pm25": air_row.get("pm25s", {}).get(zone_id),
                }
            )

    frame = pd.DataFrame.from_records(rows, columns=[TIMESTAMP_COLUMN, ZONE_ID_COLUMN, *AIR1_FEATURE_COLUMNS])
    ...
    for column in AIR1_FEATURE_COLUMNS:
...
def ensure_raw_feature_columns(frame: pd.DataFrame) -> pd.DataFrame:
    prepared = frame.copy()
    if ZONE_ID_COLUMN not in prepared.columns:
        raise ValueError(f"Frame is missing required grouping column: {ZONE_ID_COLUMN}")
    ...
    if TIMESTAMP_COLUMN in prepared.columns:
        prepared = prepared.sort_values([TIMESTAMP_COLUMN, ZONE_ID_COLUMN])
        prepared = prepared.drop_duplicates(subset=[TIMESTAMP_COLUMN, ZONE_ID_COLUMN], keep="last")
    ...
    *,
    ...
    if (sen55_path.suffix.lower() == ".csv" and csv_size_guard.has_csv_data(sen55_path)) or sen55_path.is_file():
        try:
            ...
            merged = merged.merge(sen55_by_minute, on=TIMESTAMP_COLUMN, how="left", validate="many_to_one")
...
def build_all_zones_feature_frame(
    *,
    minute_index: list[datetime],
    air_by_minute: dict[datetime, dict[str, dict[int, float]]],
    sen55_csv: Path | str,
    ...
    base = build_all_zones_base_feature_frame(minute_index, air_by_minute)
    return merge_sen55_features(base, sen55_csv, logger=logger)


def build_all_zones_feature_frame_from_histories(
    *,
    minute_index: list[datetime],
    ...
    return build_all_zones_feature_frame(
