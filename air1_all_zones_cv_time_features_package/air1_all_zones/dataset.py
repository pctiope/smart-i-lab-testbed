...
DEFAULT_TRAINING_PARQUET_DIR = PACKAGE_ROOT / "data"
DEFAULT_TRAINING_PARQUET = DEFAULT_TRAINING_PARQUET_DIR / "air1_all_zones_training_cv.parquet"
...
def _latest_training_parquet(training_parquet_dir: Path = DEFAULT_TRAINING_PARQUET_DIR) -> Path:
    if DEFAULT_TRAINING_PARQUET.is_file():
        return DEFAULT_TRAINING_PARQUET
    required_columns = {TIMESTAMP_COLUMN, ZONE_ID_COLUMN, *RAW_FEATURE_COLUMNS, TARGET_COLUMN}
...
def _clean_all_zones_training_frame(raw: pd.DataFrame, source_label: str) -> pd.DataFrame:
    target_source = _target_column_in(raw)
    required_columns = [TIMESTAMP_COLUMN, ZONE_ID_COLUMN, target_source]
    ...
    frame = frame.sort_values([ZONE_ID_COLUMN, TIMESTAMP_COLUMN])
    frame = frame.drop_duplicates(subset=[TIMESTAMP_COLUMN, ZONE_ID_COLUMN], keep="last")
    frame = frame.reset_index(drop=True)

    labeled_rows = int(frame[TARGET_COLUMN].notna().sum())
    if frame.empty or labeled_rows == 0:
        raise ValueError(
            "No labeled all-zone rows remain after parsing timestamps and target values. "
            "Rows with null feature values are retained, but supervised training needs at least one CV label."
        )
    return frame


def load_all_zones_csv(
    ...
    return _clean_all_zones_training_frame(raw, "CSV"), path


def load_all_zones_parquet(parquet_path: str | Path | None = None) -> tuple[pd.DataFrame, Path]:
    ...
    return _clean_all_zones_training_frame(raw, "Parquet"), path


def load_all_zones_training_data(
    csv_path: str | Path | None = None,
    ...
    if parquet_path is not None:
        frame, path = load_all_zones_parquet(parquet_path)
        return frame, path, "parquet"
    if csv_path is not None:
        frame, path = load_all_zones_csv(csv_path, allow_bad_lines=allow_bad_lines)
        return frame, path, "csv"
    try:
        frame, path = load_all_zones_parquet()
        return frame, path, "parquet"
    except FileNotFoundError:
        frame, path = load_all_zones_csv(allow_bad_lines=allow_bad_lines)
        return frame, path, "csv"
...
def _strict_date_coverage(
    ...
    date_series = normalized[TIMESTAMP_COLUMN].dt.normalize()
    required_rows = int(math.ceil(_rows_per_full_day() * float(min_coverage)))
    eligible: list[pd.Timestamp] = []
    excluded: list[dict[str, Any]] = []
    for date in dates:
        day_rows = normalized.loc[date_series == date]
        if ZONE_ID_COLUMN in day_rows.columns:
            zone_counts = day_rows.groupby(ZONE_ID_COLUMN)[TIMESTAMP_COLUMN].nunique()
            rows = int(zone_counts.min()) if not zone_counts.empty else 0
        else:
            rows = int(day_rows[TIMESTAMP_COLUMN].nunique())
...
def _frame_time_bounds(frame: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        "rows": int(len(frame)),
        "timestamps": int(timestamps.nunique()),
        "zones": int(frame[ZONE_ID_COLUMN].nunique()) if ZONE_ID_COLUMN in frame.columns else 0,
...
def _split_by_timestamp_fraction(frame: pd.DataFrame, validation_frac: float) -> tuple[pd.DataFrame, pd.DataFrame]:
    ordered_timestamps = pd.Series(pd.to_datetime(frame[TIMESTAMP_COLUMN]).dropna().unique()).sort_values().tolist()
    n_timestamps = len(ordered_timestamps)
    if n_timestamps < 2:
        raise ValueError(
            "Emergency validation requires at least 2 timestamps before the hidden test day; "
            f"found {n_timestamps}."
        )
    validation_timestamps = max(1, int(math.ceil(n_timestamps * float(validation_frac))))
    validation_timestamps = min(validation_timestamps, n_timestamps - 1)
    val_start_ts = pd.Timestamp(ordered_timestamps[n_timestamps - validation_timestamps])
    ts = pd.to_datetime(frame[TIMESTAMP_COLUMN])
    train_df = frame.loc[ts < val_start_ts].copy().reset_index(drop=True)
    val_df = frame.loc[ts >= val_start_ts].copy().reset_index(drop=True)
...
def chronological_validation_fold(
    ...
    train_df, val_df = _split_by_timestamp_fraction(pre_test_frame, validation_frac)
    if train_df.empty or val_df.empty:
        raise ValueError(
...
def validation_folds_for_blind_split(
    ...
    split_policy = {
        ...
        "split_grouping": "timestamp groups; all zones for one timestamp remain in the same split",
...
def chronological_split(frame: pd.DataFrame) -> dict[str, pd.DataFrame]:
    timestamps = pd.Series(pd.to_datetime(frame[TIMESTAMP_COLUMN]).dropna().unique()).sort_values().tolist()
    n_groups = len(timestamps)
    train_end = int(n_groups * TRAIN_FRAC)
    val_end = int(n_groups * (TRAIN_FRAC + VAL_FRAC))
    if train_end <= 0 or val_end <= train_end or val_end >= n_groups:
        raise ValueError(f"Chronological split produced empty timestamp group(s); groups={n_groups}")
    train_ts = set(pd.Timestamp(value) for value in timestamps[:train_end])
    val_ts = set(pd.Timestamp(value) for value in timestamps[train_end:val_end])
    test_ts = set(pd.Timestamp(value) for value in timestamps[val_end:])
    ts = pd.to_datetime(frame[TIMESTAMP_COLUMN])
    splits = {
        "train": frame.loc[ts.isin(train_ts)].copy(),
        "val": frame.loc[ts.isin(val_ts)].copy(),
        "test": frame.loc[ts.isin(test_ts)].copy(),
    }
...
def apply_training_preprocessing(frame: pd.DataFrame, fill_values: dict[str, float]) -> pd.DataFrame:
    ...
    prepared = prepared.dropna(subset=[TIMESTAMP_COLUMN, ZONE_ID_COLUMN, *FEATURE_COLUMNS])
...
def _min_zone_rows(frame: pd.DataFrame) -> int:
    counts = frame.groupby(ZONE_ID_COLUMN)[TIMESTAMP_COLUMN].count()
    return int(counts.min()) if not counts.empty else 0


def valid_lookback_candidates(splits: dict[str, pd.DataFrame]) -> list[int]:
    max_fit = min(_min_zone_rows(split) for split in splits.values())
...
def _window_labels_for_lookback(frame: pd.DataFrame, lookback: int) -> np.ndarray:
    ...
    for _zone_id, group in frame.sort_values([ZONE_ID_COLUMN, TIMESTAMP_COLUMN]).groupby(ZONE_ID_COLUMN, sort=True):
        group_labels = group[TARGET_COLUMN].to_numpy(dtype=np.float32)
        if len(group_labels) < lookback:
            continue
        window_labels = group_labels[lookback - 1 :].astype(np.float32, copy=False)
        labels.append(window_labels[~np.isnan(window_labels)])
    if not labels:
        return np.empty((0,), dtype=np.float32)
    return np.concatenate(labels).astype(np.float32, copy=False)
...
def make_windows(frame: pd.DataFrame, lookback: int) -> tuple[np.ndarray, np.ndarray, pd.Series, pd.Series]:
    X_parts: list[np.ndarray] = []
    y_parts: list[np.ndarray] = []
    ts_parts: list[pd.Series] = []
    zone_parts: list[pd.Series] = []

    ordered = frame.sort_values([ZONE_ID_COLUMN, TIMESTAMP_COLUMN]).reset_index(drop=True)
    for zone_id, group in ordered.groupby(ZONE_ID_COLUMN, sort=True):
        ...
        timestamps = group[TIMESTAMP_COLUMN].reset_index(drop=True)
        if len(group) < lookback:
            continue
        X_zone = np.lib.stride_tricks.sliding_window_view(values, window_shape=lookback, axis=0)
        X_zone = np.ascontiguousarray(X_zone, dtype=np.float32)
        y_all = labels[lookback - 1 :].astype(np.float32, copy=False)
        labeled_mask = ~np.isnan(y_all)
        if not labeled_mask.any():
            continue
        X_parts.append(X_zone[labeled_mask])
        y_parts.append(y_all[labeled_mask])
        ts_parts.append(timestamps.iloc[lookback - 1 :].reset_index(drop=True).iloc[labeled_mask].reset_index(drop=True))
        zone_parts.append(pd.Series([int(zone_id)] * int(labeled_mask.sum())))
    ...
    return (
        ...
        pd.concat(ts_parts, ignore_index=True),
...
def build_split_windows(
    ...
    for split_name, split_df in scaled_splits.items():
        X_split, y_split, ts_split, zones_split = make_windows(split_df, lookback)
        result[split_name] = {
            "X": X_split,
            "y": y_split,
            "timestamps": ts_split,
            "zone_ids": zones_split,
        }
    return result
...
def label_evidence_counts(frame: pd.DataFrame) -> dict[str, int]:
    ...
    ordered[ZONE_ID_COLUMN] = pd.to_numeric(ordered[ZONE_ID_COLUMN], errors="coerce").astype("Int64")
    ordered = ordered.dropna(subset=[TIMESTAMP_COLUMN, ZONE_ID_COLUMN])
    labels = pd.to_numeric(ordered[TARGET_COLUMN], errors="coerce")
    positives = labels == 1
    negatives = labels == 0
    ordered = ordered.assign(_positive=positives)
    events = 0
    max_gap = pd.Timedelta(seconds=SAMPLE_INTERVAL_SECONDS)
    for _zone_id, group in ordered.sort_values([ZONE_ID_COLUMN, TIMESTAMP_COLUMN]).groupby(ZONE_ID_COLUMN, sort=True):
        ...
        for _idx, row in group.iterrows():
            ...
            is_positive = bool(row["_positive"])
            gap_break = previous_ts is not None and (ts - previous_ts) > max_gap
            if is_positive and (not previous_positive or gap_break):
                events += 1
            previous_positive = is_positive
            previous_ts = ts
    return {
