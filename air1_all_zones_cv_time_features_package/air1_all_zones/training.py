...
from air1_all_zones.dataset import (
    ...
    load_all_zones_csv,
    load_all_zones_parquet,
    load_all_zones_training_data,
...
def viable_cv_lookback_candidates(
    ...
    min_rows = min(
        [_min_zone_rows(final_splits["pre_test"]), _min_zone_rows(final_splits["test"])]
        + [_min_zone_rows(fold["train"]) for fold in cv_folds]
        + [_min_zone_rows(fold["val"]) for fold in cv_folds]
    )
...
def evaluate_model(
    ...
    for split_name, payload in split_windows.items():
        ...
        if "zone_ids" in payload:
            prediction_frame[ZONE_ID_COLUMN] = pd.to_numeric(payload["zone_ids"], errors="coerce").astype("Int64")
...
def train_all_zones_from_csv(
    ...
    frame, source_data_path, source_data_format = load_all_zones_training_data(
        csv_path=csv_path,
    ...
    study_name = f"cnn_1d_air1_all_zones_training_csv_{uuid.uuid4().hex[:12]}"
    ...
    model_path = models_dir / "best_cnn_all_zones.pt"
    checkpoint = {
    ...
    _write_json(best_params_payload, tables_dir / "best_params_all_zones.json")
    _write_json(scaler_payload, tables_dir / "scaler_stats_all_zones.json")
    _write_json(metrics_payload, tables_dir / "metrics_all_zones.json")

    expected_artifacts = [
        models_dir / "best_cnn_all_zones.pt",
        tables_dir / "best_params_all_zones.json",
        tables_dir / "scaler_stats_all_zones.json",
        tables_dir / "metrics_all_zones.json",
    ]
    ...
    try:
        torch.load(models_dir / "best_cnn_all_zones.pt", map_location="cpu", weights_only=True)
    except TypeError:
        torch.load(models_dir / "best_cnn_all_zones.pt", map_location="cpu")
    ...
    return {
        ...
        "source_csv_path": portable_source_data_path if source_data_format == "csv" else None,
        "source_parquet_path": portable_source_data_path if source_data_format == "parquet" else None,
        "best_params_path": str((tables_dir / "best_params_all_zones.json").resolve()),
        "scaler_stats_path": str((tables_dir / "scaler_stats_all_zones.json").resolve()),
        "metrics_path": str((tables_dir / "metrics_all_zones.json").resolve()),
        "best_mean_cv_pr_auc": float(study.best_value),
        "best_validation_pr_auc": float(study.best_value),
        "lookback": best_lookback,
        "validation_mode": split_policy.get("validation_mode"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a AIR-1 all zones 1D CNN from generated CSV or Parquet training data.")
    source_group = parser.add_mutually_exclusive_group()
    source_group.add_argument(
        "--csv",
        type=Path,
        default=None,
        help="Training CSV path. If no input is passed, newest Parquet is preferred, then newest CSV.",
    )
    source_group.add_argument(
        "--parquet",
        type=Path,
        default=None,
        help="Training Parquet path. If no input is passed, newest Parquet is preferred, then newest CSV.",
    )
...
def main() -> None:
    args = parse_args()
    result = train_all_zones_from_csv(
        csv_path=args.csv,
