...
from air1_all_zones import feature_contract as training


PACKAGE_ROOT = Path(__file__).resolve().parent.parent
DATA_DIR = PACKAGE_ROOT / "data"

DEFAULT_FEATURES_PARQUET = DATA_DIR / "air1_all_zones_training_features.parquet"
DEFAULT_LABELS_PARQUET = DATA_DIR / "cv_occupancy_all_air1_10sec.parquet"
DEFAULT_OUTPUT_CSV = DATA_DIR / "air1_all_zones_training_cv.csv"
DEFAULT_OUTPUT_PARQUET = DATA_DIR / "air1_all_zones_training_cv.parquet"
DEFAULT_METADATA_JSON = DATA_DIR / "air1_all_zones_training_cv.metadata.json"

CV_TARGET_COLUMN = training.TARGET_COLUMN
LEGACY_CV_TARGET_COLUMNS = training.LEGACY_TARGET_COLUMNS
...
SEN55_FEATURE_COLUMNS = training.SEN55_FEATURE_COLUMNS
...
def _clean_features(raw_features: pd.DataFrame) -> pd.DataFrame:
    ...
    if training.ZONE_ID_COLUMN not in frame.columns:
        raise ValueError(f"features are missing required column: {training.ZONE_ID_COLUMN}")
    frame[training.ZONE_ID_COLUMN] = pd.to_numeric(frame[training.ZONE_ID_COLUMN], errors="coerce").astype("Int64")
    frame = frame.dropna(subset=[training.ZONE_ID_COLUMN])
    ...
    frame = frame.sort_values(["timestamp", training.ZONE_ID_COLUMN])
    frame = frame.drop_duplicates(["timestamp", training.ZONE_ID_COLUMN], keep="last")
...
def _clean_labels(raw_labels: pd.DataFrame, occupied_threshold: float) -> pd.DataFrame:
    ...
    if training.ZONE_ID_COLUMN not in frame.columns:
        raise ValueError(f"CV labels are missing required per-zone column: {training.ZONE_ID_COLUMN}")
    frame[training.ZONE_ID_COLUMN] = pd.to_numeric(frame[training.ZONE_ID_COLUMN], errors="coerce").astype("Int64")
    frame = frame.dropna(subset=[training.ZONE_ID_COLUMN])
    frame = frame.loc[frame[training.ZONE_ID_COLUMN].astype(int).isin(training.ALL_ZONE_IDS)].copy()
    ...
    if CV_TARGET_COLUMN in frame.columns:
        frame["_explicit_target"] = pd.to_numeric(frame[CV_TARGET_COLUMN], errors="coerce")
    else:
        frame["_explicit_target"] = np.nan
    for col in ["first_message_time", "last_message_time", "source_topic", "camera_ids", "label_scope", "label_source"]:
        if col not in frame.columns:
            frame[col] = None

    def join_present(values: pd.Series) -> str:
        present = [str(value) for value in values.dropna() if str(value).strip()]
        return "|".join(dict.fromkeys(present))

    def last_present(values: pd.Series) -> Any:
        present = values.dropna()
        return present.iloc[-1] if not present.empty else np.nan

    grouped = (
        frame.sort_values(["timestamp", training.ZONE_ID_COLUMN])
        .groupby(["timestamp", training.ZONE_ID_COLUMN], as_index=False)
        .agg(
            ...
            source_topic=("source_topic", join_present),
            camera_ids=("camera_ids", join_present),
            label_scope=("label_scope", last_present),
            label_source=("label_source", join_present),
            _explicit_target=("_explicit_target", last_present),
        )
    )
    grouped[CV_TARGET_COLUMN] = np.nan
    has_count = grouped["occupancy_count"].notna()
    grouped.loc[has_count, CV_TARGET_COLUMN] = (
        grouped.loc[has_count, "occupancy_count"] >= float(occupied_threshold)
    ).astype(float)
    no_count = ~has_count
    grouped.loc[no_count, CV_TARGET_COLUMN] = grouped.loc[no_count, "_explicit_target"]
    grouped["label_scope"] = grouped["label_scope"].fillna("per_zone").replace("", "per_zone")
    grouped = grouped.drop(columns=["_explicit_target"])
    return grouped[CV_LABEL_COLUMNS].sort_values(["timestamp", training.ZONE_ID_COLUMN]).reset_index(drop=True)
...
def _write_outputs(frame: pd.DataFrame, csv_path: Path, parquet_path: Path) -> None:
...
def combine_feature_label_frames(
    ...
    joined = features.merge(labels, on=["timestamp", training.ZONE_ID_COLUMN], how="left", validate="one_to_one")
    joined = _add_missing_indicators(joined)
    if CV_TARGET_COLUMN not in joined.columns:
        joined[CV_TARGET_COLUMN] = np.nan
    joined = joined[_ordered_combined_columns(joined)].sort_values(["timestamp", training.ZONE_ID_COLUMN])
...
def build_cv_training_data(
    ...
    labels_path: str | Path = DEFAULT_LABELS_PARQUET,
    output_path: str | Path = DEFAULT_OUTPUT_PARQUET,
    ...
    _write_outputs(joined, output_csv_path, output_path)
    ...
    metadata = {
        ...
        "grouping_column": training.ZONE_ID_COLUMN,
        ...
        "aggregation": {
            ...
            "sen55": "shared per-10-second mean copied to each zone row",
            "cv_occupancy_count": "per-zone per-10-second median joined by timestamp and zone_id",
        },
        "label_scope": "per_zone",
        "occupied_rule": (
            f"{CV_TARGET_COLUMN}=1 when the per-zone median CV occupancy_count >= {occupied_threshold:g}; "
            f"{CV_TARGET_COLUMN}=0 when the per-zone median CV occupancy_count < {occupied_threshold:g}; "
            f"{CV_TARGET_COLUMN}=null when no CV label exists for that timestamp + zone_id"
        ),
        "join": {
        ...
        "outputs": {
            ...
            "parquet_path": _portable_path(output_path),
            "parquet_sha256": _sha256_of_file(output_path),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build one null-preserving long-form all-zone AIR-1 + SEN55 + CV training table."
    )
    parser.add_argument("--features", type=Path, default=DEFAULT_FEATURES_PARQUET)
    parser.add_argument("--labels", type=Path, default=DEFAULT_LABELS_PARQUET)
