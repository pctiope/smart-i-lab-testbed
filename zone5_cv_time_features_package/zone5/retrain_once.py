...
DEFAULT_SOURCE_PARQUET = PACKAGE_ROOT / "data" / "zone5_training_cv.parquet"
DEFAULT_SOURCE_CSV = PACKAGE_ROOT / "data" / "zone5_training_cv.csv"
DEFAULT_SOURCE_METADATA = PACKAGE_ROOT / "data" / "zone5_training_cv.metadata.json"
...
def _snapshot_training_source(args: argparse.Namespace) -> dict[str, Any]:
    ...
    stem = f"zone5_training_cv_{timestamp}_{uuid.uuid4().hex[:8]}"
    ...
    if snapshot_source == "csv":
        ...
        frame, _path = training.load_zone_5_csv(source, allow_bad_lines=args.allow_bad_lines)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one Zone 5 retraining cycle from a stable CSV or Parquet snapshot."
    )
    ...
    parser.add_argument(
        ...
        help="Minimum positive 10-second blind-test buckets needed before running a promotable retrain. Default: 5.",
    )
    parser.add_argument(
        "--min-positive-events",
        type=int,
        default=int(os.environ.get("ZONE5_MIN_POSITIVE_EVENTS", str(DEFAULT_MIN_POSITIVE_EVENTS))),
        help="Minimum contiguous positive blind-test events needed before running a promotable retrain. Default: 1.",
    )
    parser.add_argument(
