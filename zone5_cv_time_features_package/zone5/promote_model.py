...
def _load_run_payload(run_dir: Path) -> dict[str, Any]:
    ...
    metrics = _load_json(run_dir / "tables" / "metrics_zone_5.json")
...
def _load_run_policy_payload(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        "manifest": _load_json(manifest_path) if manifest_path.is_file() else {},
        "metrics": _load_json(tables_dir / "metrics_zone_5.json")
        if (tables_dir / "metrics_zone_5.json").is_file()
        else {},
        "scaler": _load_json(tables_dir / "scaler_stats_zone_5.json")
        if (tables_dir / "scaler_stats_zone_5.json").is_file()
        else {},
        "params": _load_json(tables_dir / "best_params_zone_5.json")
        if (tables_dir / "best_params_zone_5.json").is_file()
        else {},
    }
...
def _candidate_blind_test_frame(payload: dict[str, Any]) -> pd.DataFrame | None:
    ...
    if source_format == "csv":
        frame, _path, _format = training.load_zone_5_training_data(csv_path=source_path)
    else:
        frame, _path, _format = training.load_zone_5_training_data(parquet_path=source_path)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Promote a valid CV-target Zone 5 run to production.")
    parser.add_argument(
        "--candidate-run",
        type=Path,
    ...
    parser.add_argument(
        "--min-mean-cv-pr-auc",
        type=float,
        default=float(os.environ.get("ZONE5_MIN_MEAN_CV_PR_AUC", "0.0")),
        help="Minimum rolling-CV mean PR-AUC. Default 0.",
    )
    parser.add_argument(
        "--min-pr-auc-delta",
        type=float,
        default=float(os.environ.get("ZONE5_MIN_PR_AUC_DELTA", "0.0")),
    ...
    parser.add_argument(
        ...
        help="Minimum positive 10-second blind-test buckets. Default 5.",
    )
    parser.add_argument(
        "--min-positive-events",
        type=int,
        default=int(os.environ.get("ZONE5_MIN_POSITIVE_EVENTS", "1")),
        help="Minimum contiguous positive blind-test events. Default 1.",
    )
    parser.add_argument(
        "--max-test-brier",
        type=float,
