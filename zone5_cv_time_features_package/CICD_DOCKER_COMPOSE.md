...
`LIVE DATA DEGRADED: core sensor coverage below gate`, the core coverage gate
failed. AIR-1 and power fields require at least `0.80` coverage, and
`mmwave_s5` requires at least `0.95` coverage. `sen55-missing` is not the
blocker by itself because SEN55 is optional. The core AIR-1, smart plug, and
mmWave fields gate live prediction. Inspect `/api/current.error`, verify
upstream Smart I-Lab API history for the Zone 5 devices, and restart only the
live app service if upstream data is healthy but the running app cache remains
stale.
